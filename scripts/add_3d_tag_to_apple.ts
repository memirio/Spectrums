...
(async () => {
  const apple = await prisma.site.findFirst({
    where: { url: { contains: 'apple.com/airpods-pro' } },
    include: { images: true }
  });

  if (!apple || !apple.images[0]) {
    console.log('Site not found');
    return;
  }

  const img = apple.images[0];
  ...
  console.log(`Adding 3D tag to apple.com/airpods-pro:`);
