...
async function processSite(url: string) {
  try {
    const title = new URL(url).hostname.replace('www.', '')
    
    // Use the API route which handles the full pipeline
    ...
    try {
      ...
      return { success: true, hasImage: !!data.site?.imageUrl, url }
    } catch (fetchError) {
      clearTimeout(timeoutId)
      throw fetchError
    }
...
async function main() {
  console.log(`Processing ${urls.length} sites via API...\n`)

  const results = []
  for (let i = 0; i < urls.length; i++) {
    const url = urls[i]
    console.log(`[${i + 1}/${urls.length}] Processing ${url}...`)
    const result = await processSite(url)
    results.push(result)
    
    if (result.success) {
      console.log(`  ✓ ${result.hasImage ? 'Screenshot generated' : 'Queued for processing'}`)
    } else {
      console.log(`  ✗ ${result.error}`)
    }
    
    // Delay between sites to avoid overwhelming the server
