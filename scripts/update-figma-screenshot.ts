...
async function updateFigma() {
  try {
    const site = await prisma.site.findFirst({ 
      where: { url: { contains: 'figma' } } 
    })
    
    if (!site) {
      console.log('❌ Figma site not found')
      return
    }
    ...
    const response = await fetch(`${SCREENSHOT_API_URL}/api/screenshot`, {
      ...
      headers: { 
        ...
        'Idempotency-Key': `figma-${Date.now()}` 
      },
      body: JSON.stringify({ 
        url: site.url, 
    ...
    if (data.statusUrl) {
      console.log('⏳ Waiting for screenshot (this may take longer for Figma)...')
      for (let i = 0; i < 30; i++) {
        ...
        if (i % 5 === 0) console.log(`   Still processing... (${i * 2}s)`)
      }
    }
    ...
    if (imageUrl) {
      ...
      console.log('✅ Updated Figma screenshot in database')
    } else {
