...
async function updateAirbnb() {
  try {
    const site = await prisma.site.findFirst({ 
      where: { url: { contains: 'airbnb' } } 
    })
    
    if (!site) {
      console.log('❌ Airbnb site not found')
      return
    }
    ...
    const response = await fetch(`${SCREENSHOT_API_URL}/api/screenshot`, {
      ...
      headers: { 
        ...
        'Idempotency-Key': `airbnb-viewport-${Date.now()}` 
      },
      body: JSON.stringify({ 
        url: site.url, 
    ...
    if (imageUrl) {
      ...
      console.log('✅ Updated Airbnb screenshot in database')
    } else {
