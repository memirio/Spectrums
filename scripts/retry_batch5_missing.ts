#!/usr/bin/env tsx
/**
 * Retry Missing Websites from Batch 5
 * 
 * Retries uploading the batch 5 websites that failed due to database overload.
...
// All batch 5 websites
const BATCH5_WEBSITES = [
...
async function processEntryWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        const delay = Math.min(10000 * Math.pow(2, attempt - 1), 60000) // Exponential backoff, max 60s
...
async function main() {
  console.log('🚀 Retrying batch 5 websites with improved error handling...\n')
  console.log(`📁 Image directory: ${IMAGE_DIR}`)
  console.log(`🌐 API URL: ${API_URL}`)
  console.log(`📊 Total entries: ${BATCH5_WEBSITES.length}\n`)
  ...
  const entriesWithImages = BATCH5_WEBSITES.filter(entry => {
    const imagePath = findImageFile(entry.url, imageFiles)
    return imagePath !== null && fs.existsSync(imagePath)
  })
  
  console.log(`📋 Found ${entriesWithImages.length} entries with matching images (out of ${BATCH5_WEBSITES.length} total)\n`)
  
  // Process each entry with longer delays (10 seconds between entries)
  for (let i = 0; i < entriesWithImages.length; i++) {
    ...
    // 10 second delay between entries to avoid overwhelming the database
    if (i < entriesWithImages.length - 1) {
      console.log(`  ⏳ Waiting 10 seconds before next entry...`)
