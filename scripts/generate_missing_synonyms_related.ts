...
/**
 * Generate synonyms and related terms for a concept using Groq
 */
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit && attempt < maxRetries - 1) {
        ...
        const waitSeconds = waitMatch ? parseFloat(waitMatch[1]) : Math.pow(2, attempt) * 10 // Exponential backoff: 10s, 20s, 40s
...
async function main() {
  ...
  console.log('🔄 Generating Missing Synonyms and Related Terms')
  console.log('═'.repeat(70))
  console.log()
  ...
  if (conceptsNeedingUpdate.length === updated) {
